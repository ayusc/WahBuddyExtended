...
class XposedInit: IXposedHookLoadPackage {
				...
				object: XC_MethodHook() {
					override fun afterHookedMethod(param: MethodHookParam) {
						if (!isLoopActive) {
							...
							val lastLogBootTime = prefs.getLong("lastLogBoot", 0 L)
	...
	private fun syncAboutLoop(app: Application, classLoader: ClassLoader, shouldLog: Boolean) {
			...
			override fun run() {
				executor.execute {
					try {
						...
						XposedHelpers.callMethod(dispatcher, "A05", quote, emoji, depO1, depO2, 86400 L, false)
				...
				val delayToNextMinute = (60 - currentSeconds) * 1000 L
		...
		val calendar = Calendar.getInstance(TimeZone.getTimeZone("Asia/Kolkata"))
		val currentSeconds = calendar.get(Calendar.SECOND)
		val initialDelay = (60 - currentSeconds) * 1000 L
